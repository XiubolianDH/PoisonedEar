...
def _audio_as_data_url(audio_path: str) -> str:
...
class GeminiAudioModel(VictimModel):
    ...
    def generate(self, model_input: ModelInput) -> ModelOutput:
        ...
        return ModelOutput(model_name=self.model_name, text=response.text, raw_response=response)


class QwenOmniModel(VictimModel):
    def __init__(
        self,
        model_name: str,
        api_base_env: str,
        ...
        if not api_base or not api_key:
            raise EnvironmentError(f"Missing Qwen Omni connection env vars {api_base_env} or {api_key_env}")
...
def build_victim_model(config: dict) -> VictimModel:
