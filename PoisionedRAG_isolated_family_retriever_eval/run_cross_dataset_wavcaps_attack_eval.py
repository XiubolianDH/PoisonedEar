...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--retriever-encoder", default=None, help="Retriever encoder to use (e.g. clap, audioclip, panns, wav2vec2).")
...
def main() -> None:
    ...
    retriever = FaissAudioRetriever(encoder=encoder, similarity=similarity)
    ...
    try:
        for query in query_targets:
            try:
                ...
                recall_at_k = float(any(hit.sample.text == query["clean_caption"] for hit in hits))
                ...
                audio_embedding = encoder.encode_audio([query["audio_path"]])[0]
