...
def main() -> None:
    ...
    if all_rows:
        asr_r_values = [float(row["asr_r"]) for row in all_rows]
        asr_g_values = [float(row["asr_g"]) for row in all_rows]
        asr_joint_values = [r * g for r, g in zip(asr_r_values, asr_g_values)]
        retrieved_rows = [g for r, g in zip(asr_r_values, asr_g_values) if r > 0]
        summary = {
            "num_queries": len(all_rows),
            "top_k": args.top_k,
            "retriever": encoder.name,
            "overall_asr": float(np.mean(asr_joint_values)),
            "overall_asr_definition": "P(ASR-R and ASR-G)",
            "recall_at_k": float(np.mean([float(row["recall@k"]) for row in all_rows])),
            "recall_definition": "malicious_fraction_in_top_k",
            "asr_r_rate": float(np.mean(asr_r_values)),
            "asr_g_rate": float(np.mean(asr_g_values)),
            "asr_g_given_r_rate": float(np.mean(retrieved_rows)) if retrieved_rows else 0.0,
            "asr_g_given_r_num_queries": len(retrieved_rows),
