...
MALICIOUS_METADATA = Path("malicious_dataset/unified_vehicle_transport_20/final_dataset/metadata.json")
OUTPUT_DIR = Path("attack_experiments/wavcaps_vehicle_transport_rates")
RATES = [1, 5, 10, 15, 20]
SEED = 7
...
def main() -> None:
    ...
    for rate in RATES:
        poison_count = min(len(ordered_malicious), int(math.floor(101 * (rate / 100.0))))
            ...
            augmented_classified.append(
                {
                    ...
                    "category": "vehicle_transport_poison",
                    "split": "poison",
                    "primary_class": "vehicle_transport",
                    "matched_classes": ["vehicle_transport"],
                    "matched_keywords": ["vehicle_transport_poison"],
                    "is_poisoned": True,
                    "poison_metadata": {
        ...
        subset_payload = {
            "dataset_name": f"wavcaps_vehicle_transport_{rate:02d}pct",
    ...
    (OUTPUT_DIR / "experiment_plan.json").write_text(
        json.dumps({"seed": SEED, "rates": plan_rows}, ensure_ascii=True, indent=2),
        encoding="utf-8",
    )
